...
4. **Dismantle `Modules/User/` TPH.** Replace with flat per-service profile tables: `VenueManagerProfile { Sub, VenueId }` and `ArtistManagerProfile { Sub, ArtistId }` and `AdminProfile { Sub }` in B2B; `CustomerProfile { Sub }` in Customer. "Role" is a B2B concept — not an identity concept. Auth (Duende IS) keeps user accounts + credential storage + token issuance; it drops role enforcement (that's Step 5). Per-service authorization: token arrives → service looks up `sub` in its own profile table → 403 if not found. VenueManager = the profile row itself; tenant relationship is the `VenueId` FK. Sequenced carefully so the monolith builds at each step (R6).
