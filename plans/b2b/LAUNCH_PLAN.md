...
- **DoorSplit/Versus revenue source** — these two settle against door/ticket revenue, which standalone B2B has no feed for. Money mechanic is identical for any non-owned-checkout option: Concertable charges the venue for the artist's share and settles it through Stripe (as FlatFee escrow already does); only *owned checkout* puts Concertable in the flow of the ticket funds directly. Options:
  ...
  **Confirmed (2026-06-22): (a) manual door-take entry + charge-the-venue for v1 — ships all four contract types in the pure-B2B MVP — + (c) owned checkout as the deferred durable feed; (b) ruled out.** Settlement UI needs a door-take entry screen at settlement for DoorSplit/Versus; the money mechanic reuses FlatFee escrow.
- **Revenue model** — per-gig fee / subscription / % commission / hybrid. Decide by end of Month 1. **Settled principle (2026-06-22):** the fee rides on the *settlement transaction*, which must route through our Stripe Connect — charge the venue for the artist's share **+ our fee**, then pay the artist; same mechanic for all four contract types regardless of where tickets sold. The model to avoid is fully invoice-only/GigPig-style (money moves venue→artist directly), which leaves no transaction to take a cut from. Corollary: **we never force venues onto our checkout** — the own marketplace is an *additive second distribution option*, not a prerequisite for monetization or door-split/versus settlement (manual door-take entry + charge-the-venue covers both). Owning the checkout only *upgrades* it (verified sales number, no venue credit risk, fee taken at point of sale).
...
- **2026-06-22 (monetization principle)** — Settled how Concertable earns: the fee rides on the *settlement transaction* that routes through our Stripe Connect (charge the venue for the artist's share + our fee → pay the artist), identical for all four contract types whether tickets sold on our checkout, an external ticketer, or the door. The model to avoid is fully invoice-only (GigPig-style, money venue→artist directly) — no transaction to take a cut from. Corollary: the own marketplace is an additive *second* distribution option, never forced and **not** a prerequisite for monetization or door-split/versus settlement; owning the checkout only upgrades it (verified number, no venue credit risk, fee at point of sale). This reinforces — and lowers the liability of — the B2B-first MVP / marketplace-deferred decision (2026-05-18): a pure-B2B MVP (GigXchange-style booking + typed contracts, no consumer ticket sales) carries no consumer-protection / CMA secondary-ticketing / consumer-fund-holding exposure, and the marketplace is layered in later as the second option.
- **2026-06-22 (MVP scope confirmed)** — The v1 MVP is **pure B2B competing head-on with GigPig/GigXchange** on the one thing they structurally can't do: settle the revenue-share contract types (DoorSplit/Versus). A flat-fee-only MVP would *not* differentiate (both competitors already auto-pay flat fees), so DoorSplit/Versus must be sellable at v1 — confirmed via **manual door-take entry + charge-the-venue** (no ticketing ownership needed), which ships all four contract types without a marketplace. Marketplace stays deferred as the additive *second* distribution option (verified-number/no-credit-risk upgrade for DoorSplit/Versus). Updated §1 (differentiation thesis + DoorSplit/Versus scope now resolved, not open) and §9 (lean → confirmed) accordingly. Remaining open decision: the Revenue-model specifics, constrained by the monetization principle above (fee rides on the settlement transaction).
